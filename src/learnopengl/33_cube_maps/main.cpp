...
class Sphere {
  ...
  unsigned int light_VBO, light_VAO, light_EBO;
  Sphere(){
    	//生成球的顶点
...
int main(int argc, char *argv[])
{
  ...
  // glfw: initialize and configure
    ...
    Shader sphereshader("./shader/sphere_vert.glsl", "./shader/sphere_frag.glsl");
    Shader skyboxShader("./shader/skybox_vert.glsl", "./shader/skybox_frag.glsl");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model=rotate(model,angle,vec3(sin(angle),sin(angle+2.0),1));
        ...
        //model=scale(model,vec3(0,0.3,0.3));
        ...
        skyboxShader.setVec3("eyepos",camera.Position);
        // skybox cube
        glBindVertexArray(skyboxVAO);
        glActiveTexture(GL_TEXTURE0);
    ...
    // optional: de-allocate all ./static once they've outlived their purpose:
    // ------------------------------------------------------------------------
