...
public:
    ...
    void bindData(bool finalPass = false) {
        ...
        if (!finalPass) {
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 , GL_TEXTURE_2D, colorAttachments, 0);// 将颜色纹理绑定到 i 号颜色附件            
        }
        
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    }
    void draw(GLuint texPassArray =NULL) {
        glUseProgram(program);
        glBindFramebuffer(GL_FRAMEBUFFER, FBO);
        glBindVertexArray(vao);
        // 传上一帧的帧缓冲颜色附件
        if(texPassArray!=NULL){
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, texPassArray);
            glUniform1i(glGetUniformLocation(program, "texPass0"), 0);
        }
...
int buildBVH(std::vector<Triangle>& triangles, std::vector<BVHNode>& nodes, int l, int r, int n) {
    ...
    if (lenx >= leny && lenx >= lenz)
        sort(triangles.begin() + l, triangles.begin() + r + 1, cmpx);
    // 按 y 划分
    if (leny >= lenx && leny >= lenz)
        sort(triangles.begin() + l, triangles.begin() + r + 1, cmpy);
    // 按 z 划分
    if (lenz >= lenx && lenz >= leny)
        sort(triangles.begin() + l, triangles.begin() + r + 1, cmpz);
    // 递归
    int mid = (l + r) / 2;
...
int main(int argc, char *argv[]){
    ...
    vec3 emissive = vec3(0, 0, 0);  // 作为光源时的发光颜色
    vec3 baseColor = vec3(1, 1, 1);
    float subsurface = 0.0; //次表面散射参数,一般用不到
    float metallic = 0.0; //金属度，决定了漫反射的比例
    float specular = 0.0; //镜面反射强度控制
    float specularTint = 0.0; //控制镜面反射的颜色
   ...
   */
    Material m;
    /* m.baseColor=vec3(0.63f, 0.065f, 0.05f);
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\left.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
    m.baseColor=vec3(0.14f, 0.45f, 0.091f);
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\right.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
    m.baseColor=vec3(0.725f, 0.71f, 0.68f);
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\floor.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\shortbox.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\tallbox.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
     
    m.baseColor=vec3(0.65f);
    m.emissive=8.0f * vec3(0.747f+0.058f, 0.747f+0.258f, 0.747f) + 15.6f * vec3(0.740f+0.287f,0.740f+0.160f,0.740f) + 18.4f *vec3(0.737f+0.642f,0.737f+0.159f,0.737f);
    readObj("src\\summary\\ray_tracing\\models\\cornellbox\\light.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(0.0, 0.0, 0.0)), false);//18.83
    */
    m.baseColor=vec3(0.5, .5, .5); 
    m.subsurface=0.9;
    m.metallic=0.9;
    m.specular=0.9;
    //m.specularTint=0.2;
    ...
    //m.emissive = vec3(0.01, 0.01, 0.01);

    //readObj("src\\summary\\ray_tracing\\models\\Stanford_Bunny.txt", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0.3, -2.0, -0.5), vec3(1.5, 1.5, 1.5)),true);
    readObj("src\\summary\\ray_tracing\\models\\teapot.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(1, -1.5, -2), vec3(1.5,1.5,1.5)),true);
    m.baseColor=vec3(0.2, .2, .8); 
    m.subsurface=0.9;
    m.metallic=0.9;
    m.specular=0.9;
    //m.specularTint=0.2;
    m.roughness=0.1;
    readObj("src\\summary\\ray_tracing\\models\\sphere2.txt", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(-1, -1., -1), vec3(1.5,1.5,1.5)),true);
    
    m.baseColor=vec3(0.8, .8, .8); 
    m.subsurface=0.9;
    m.metallic=0.9;
    m.specular=0.9;
    //m.specularTint=0.2;
    m.roughness=0.1;
    readObj("src\\summary\\ray_tracing\\models\\Stanford_Bunny.txt", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0., -1), vec3(1.5,1.5,1.5)),true);
    ...
    m1.metallic=0.1;
    m1.specular=0.1;
    m1.baseColor = vec3(0.01, 0.8, 0.01);
    readObj("src\\summary\\ray_tracing\\models\\right.obj", triangles, m1, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(4.0, 4.0, 4.0)), false);//18.83
    m1.baseColor = vec3(0.01, 0.8, 0.8);
    readObj("src\\summary\\ray_tracing\\models\\up.obj", triangles, m1, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(4.0, 4.0, 4.0)), false);//18.83
    m1.metallic=0.9;
    m1.subsurface=0.9;
    m1.specular=0.8;
    m1.roughness=0.25;
    m1.baseColor = vec3(1, 1, 0.01);
    readObj("src\\summary\\ray_tracing\\models\\left.obj", triangles, m1, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(4.0, 4.0, 4.0)), false);//18.83
    m1.baseColor = vec3(0.01, 0.01, 0.8);
    
    readObj("src\\summary\\ray_tracing\\models\\down.obj", triangles, m1, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(4.0, 4.0, 4.0)), false);//18.83
    m1.baseColor = vec3(0.9, 0.01, 0.01);
    readObj("src\\summary\\ray_tracing\\models\\back.obj", triangles, m1, getTransformMatrix(vec3(0, 0, 0), vec3(0, 0, 0), vec3(4.0, 4.0, 4.0)), false);//18.83
     */
    Material m2;
    m2.baseColor = vec3(1, 1, 1);
    m2.emissive = vec3(5, 5, 5);
    
    //readObj("src\\summary\\ray_tracing\\models\\plane.obj", triangles, m2, getTransformMatrix(vec3(0, 0, 0), vec3(0, 1.5, 0), vec3(1,0.1, 1)), false);
    //readObj("src\\summary\\ray_tracing\\models\\sphere.txt", triangles, m2, getTransformMatrix(vec3(0, 0, 0), vec3(1.0, 1.0, 0), vec3(1.5,1.5, 1.5)), false);
    //readObj("src\\summary\\ray_tracing\\models\\sphere.txt", triangles, m2, getTransformMatrix(vec3(0, 0, 0), vec3(-0.3, 0.6, 0.3), vec3(0.3,0.3, 0.3)), false);

    int nTriangles = triangles.size();
    std::cout << "triangles num: " << nTriangles << std::endl;
    ...
    std::cout << "BVH v nums: " << nNodes <<  std::endl;
    ...
    bool r = HDRLoader::load("src\\summary\\ray_tracing\\HDR\\rooitou_park_4k.hdr", hdrRes);
    hdrMap = getTextureRGB32F(hdrRes.width, hdrRes.height);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, hdrRes.width, hdrRes.height, 0, GL_RGB, GL_FLOAT, hdrRes.cols);

    pass1.program = getShaderProgram("src\\summary\\ray_tracing\\shaders\\fshader3hdr.txt","src\\summary\\ray_tracing\\shaders\\vshader.txt");
    //pass1.width = pass1.height = 256;
    pass1.colorAttachments=getTextureRGB32F(pass1.width, pass1.height);
    pass1.bindData();
    ...
    pass2.program = getShaderProgram("src\\summary\\ray_tracing\\shaders\\pass2.txt","src\\summary\\ray_tracing\\shaders\\vshader.txt");
    lastFrame = getTextureRGB32F(pass2.width, pass2.height);
    pass2.colorAttachments=lastFrame;
    pass2.bindData();

    pass3.program = getShaderProgram("src\\summary\\ray_tracing\\shaders\\pass3.txt","src\\summary\\ray_tracing\\shaders\\vshader.txt");
    pass3.bindData(true);
    // ----------------------------------------------------------------------------- //
    std::cout << "start..." << std::endl << std::endl;

    glEnable(GL_DEPTH_TEST);  // 开启深度测试
    glClearColor(0.3, 0.2, 0.0, 1.0);   // 背景颜色 -- 黑
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        fps = 1.0 / dt;
        std::cout << "\r";
        std::cout << std::fixed << std::setprecision(2) << "FPS : " << fps << " cnts: " << frameCounter;
        t1 = t2;
        // 相机参数
        ...
        glUniformMatrix4fv(glGetUniformLocation(pass1.program, "cameraRotate"), 1, GL_FALSE, value_ptr(inverse(camera.GetViewMatrix())));
        ...
        if(ifclear) {
            glBindFramebuffer(GL_FRAMEBUFFER, lastFrame);
